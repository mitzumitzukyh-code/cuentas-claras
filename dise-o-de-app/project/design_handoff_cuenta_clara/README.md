...
## Respuestas a la auditoría (decisiones tomadas)

**Resueltos en el diseño (ya en los archivos):**
1. **Reportes** — nueva pantalla en `Lote N` (comparativo Semana/Mes/Año, gráfica de trazo dibujado a mano, top vendidos, Exportar a Excel/PDF).
2. **Pantalla "Más"** — nueva en `Lote N`: cajón agrupado (Dinero / Negocio / Crecer / Cuenta) con los 15 destinos + selector de negocio arriba.
3. **Bottom-nav = 5 pestañas** (Inicio · Ventas · Mercancía · Reportes · **Más**), ya corregida en los dashboards de Lote A y J. La tab actual del código (Inicio·Cobrar·Productos·Perfil) se reemplaza por esta.
4. **Multi-negocio** — selector añadido en el header del dashboard (A y J, nombre con chevron) y en el header de "Más" (Lote N).
5. **Rubros alineados al código**: Bodega · Ropa · Belleza · **Quincallería** · **Comida rápida** · Otro (Lote F). Enum destino: `bodega, ropa, belleza, quincallería, comida_rapida, otro` — sin migración si se respeta.
6. **Variantes y vencimiento** — añadidos a Nuevo producto (Lote C), marcados como campos por rubro (variantes: Ropa/Belleza/Quincallería; vencimiento: Belleza/Alimentos).
7. **Destructivo (híbrido, aceptado):** ámbar `#b07d1e` para lo **reversible** (anular venta, descuadre, stock bajo); rojo `#C74A3A` solo para lo **irreversible** (eliminar cuenta, Lote E). El rojo de marca sigue siendo el vinotinto del degradado, distinto del rojo destructivo.
8. **Precio Plus** — microcopy corregido (Lote D): el cobro lo hace Google Play en la moneda de la cuenta, NO a la tasa BCV/Binance de la app.
9. **IVA** — fila "Impuestos (IVA)" retirada de Ajustes (no hay pantalla ni modelo).
10. **Formato de moneda — regla única:** USD con punto decimal y coma de miles (`$1,284,590.50`); Bs con punto de miles y coma decimal (`Bs 177.568,80`). Todo el contenido nuevo la cumple. **Tarea dev:** normalizar montos USD heredados que aún usan coma (revisar Lote C gastos, H, y cifras sueltas) a esta regla.
11. **Toques ≥54px** en filas de lista (aplicado en B, N; estándar para toda la app) y **renglón a `.09–.1`** para contraste AA a plena luz.

**Requieren decisión de negocio/presupuesto (NO son diseño):**
- **Login WhatsApp OTP** — el diseño (Lote A) asume código por WhatsApp (como Treinta). Firebase Auth no lo soporta: requiere proveedor (Twilio/360dialog) + WhatsApp Business + Cloud Function con custom tokens. **Si no se aprueba presupuesto, cambiar copy a SMS** (Firebase ya lo soporta). Decisión pendiente del dueño del producto.
- **Pedidos por WhatsApp automáticos (Plus)** — es el beneficio #1 del plan; NO es pantalla, es un bot con webhook + parser + estado de conversación (fuera de alcance en CLAUDE.md §7). O se construye el servicio, o se ajusta el pitch del Plus antes de publicar en Play Store.
- **Descuadre de cierre de caja** — definir regla: se registra en el cierre como "diferencia" (no como gasto); se permite cerrar con descuadre pero queda anotado y visible en el reporte. Confirmar con el dueño.
- **Modelo de datos nuevo** (fiados: clientes/movimientos/abonos; proveedores; cierres; métodos de pago) — definir en CLAUDE.md §4 + reglas de seguridad Firestore ANTES de codificar.
- **Neumorfismo → plano:** el sistema libreta es plano; el código actual es neumórfico. Recomendación de diseño: **eliminar el neumorfismo** (el relieve compite con la textura de papel) y re-skinear las ~30 pantallas. Es el 70% del esfuerzo, pero es la decisión correcta para no tener dos lenguajes visuales peleando.

**Nota:** falta el archivo de Reportes original que el README anterior referenciaba como `Cuenta Clara.dc.html`; queda **sustituido por `Lote N`**, que es la fuente de verdad de Reportes y "Más".
